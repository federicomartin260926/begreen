...
final readonly class WasteEmissionRecordService
{
    public function __construct(
        private WasteEmissionCalculator $calculator,
        private WasteEmissionSnapshot $snapshot,
    ...
    public function write(
        ...
        array $presentation = [],
    ): WasteEmissionRecordWriteResult {
        if (null === $input->startDate) {
            throw new \InvalidArgumentException('startDate is required to persist a waste record.');
        }

        $calculation = $this->calculator->calculate($input);
        if (EmissionRecord::STATUS_PENDING_DATA === $calculation->status) {
            throw new \InvalidArgumentException('Incomplete or invalid waste input cannot be persisted.');
        }
        ...
        return new WasteEmissionRecordWriteResult($calculation, $record);
    }
}
