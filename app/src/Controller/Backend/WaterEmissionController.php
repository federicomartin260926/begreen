...
#[IsGranted('ROLE_USER')]
final class WaterEmissionController extends AbstractController
{
    ...
    #[Route('/water/preview', name: 'backend_emission_water_v1_preview', methods: ['POST'])]
    public function preview(
        Request $request,
        ActiveProjectService $activeProjectService,
        WaterEmissionRequestMapper $requestMapper,
        WaterEmissionCalculator $calculator,
    ): JsonResponse {
        $project = $this->activeProject($activeProjectService);
        $this->denyAccessUnlessGranted(ProjectVoter::EDIT, $project);

        $token = $request->request->get('_preview_token');
        if (!is_string($token) || !$this->isCsrfTokenValid('water_emission_v1_preview', $token)) {
    ...
    #[Route('/new-water-v1', name: 'backend_emission_new_water_v1', methods: ['GET', 'POST'])]
    public function create(
        ...
        if (!$this->hasValidCsrfToken($request, 'water_emission_v1_create')) {
    ...
    #[Route('/{id}/edit-water-v1', name: 'backend_emission_edit_water_v1', methods: ['GET', 'POST'])]
    public function edit(
        ...
        $category = $this->waterCategory($categoryRepository);
        if (!$snapshot->isWaterV1Record($record, (int) $category->getId())) {
            throw $this->createNotFoundException('Water v1 record not found.');
        }

        try {
            $storedValues = $snapshot->inputToArray($snapshot->decodeInput((string) $record->getCalculationDetails()));
        } catch (\JsonException|\UnexpectedValueException) {
            throw $this->createNotFoundException('Invalid water v1 snapshot.');
        }
        ...
        if (!$this->hasValidCsrfToken($request, 'water_emission_v1_edit_'.$record->getId())) {
        ...
        $this->addFlash('success', 'backend.emission.water_v1.flash.updated');

        return $this->redirectToRoute('backend_emission_index', $this->indexQuery($request, (int) $category->getId()));
    }

    #[Route('/{id}/duplicate-water-v1', name: 'backend_emission_duplicate_water_v1', methods: ['GET'])]
    ...
    private function renderForm(
        ...
        $formAction = $edit && null !== $record
            ? $this->generateUrl('backend_emission_edit_water_v1', array_merge(['id' => $record->getId()], $backQuery))
            : $this->generateUrl('backend_emission_new_water_v1', $backQuery);

        return $this->render('backend/emission/water_v1_form.html.twig', [
            'project' => $project,
            'category' => $category,
            'edit' => $edit,
            'duplicate' => $duplicate,
            ...
            'waterUseTypes' => WaterEmissionInput::waterUseTypes(),
            'destinations' => WaterEmissionInput::destinations(),
            'csrfTokenId' => $edit ? 'water_emission_v1_edit_'.$record?->getId() : 'water_emission_v1_create',
            'errors' => $errors,
            'backQuery' => $backQuery,
    ...
    private function waterCategory(CategoryRepository $repository): Category
    {
        $category = $repository->findOneBy(['name' => 'Agua']);
        if (!$category || !$category->isEnabledInEmissionCalculator()) {
            throw $this->createNotFoundException('Water category not found.');
        }
