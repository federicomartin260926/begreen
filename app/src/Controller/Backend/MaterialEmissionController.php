...
final class MaterialEmissionController extends AbstractController
{
    ...
    public function preview(
        ...
        MaterialEmissionRequestMapper $requestMapper,
        MaterialEmissionCalculator $calculator,
    ): JsonResponse {
        $project = $this->activeProject($activeProjectService);
        $this->denyAccessUnlessGranted(ProjectVoter::EDIT, $project);
        $token = $request->request->get('_preview_token');
        if (!is_string($token) || !$this->isCsrfTokenValid('material_emission_v1_preview', $token)) {
    ...
    #[Route('/new-material-v1', name: 'backend_emission_new_material_v1', methods: ['GET', 'POST'])]
    public function create(
        ...
        MaterialEmissionRequestMapper $requestMapper,
        MaterialEmissionRecordService $recordService,
        ...
        if (!$this->hasValidCsrfToken($request, 'material_emission_v1_create')) {
    ...
    #[Route('/{id}/edit-material-v1', name: 'backend_emission_edit_material_v1', methods: ['GET', 'POST'])]
    public function edit(
        ...
        MaterialEmissionRequestMapper $requestMapper,
        MaterialEmissionRecordService $recordService,
        MaterialEmissionSnapshot $snapshot,
        MaterialUiCatalog $catalog,
        EmissionRecordAttachmentStorage $attachmentStorage,
        EntityManagerInterface $entityManager,
    ): Response {
        ...
        if (!$snapshot->isMaterialV1Record($record, (int) $category->getId())) {
            throw $this->createNotFoundException('Material v1 record not found.');
        }
        ...
        $this->addFlash('success', 'backend.emission.material_v1.flash.updated');

        return $this->redirectToRoute('backend_emission_index', $this->indexQuery($request, (int) $category->getId()));
    }

    #[Route('/{id}/duplicate-material-v1', name: 'backend_emission_duplicate_material_v1', methods: ['GET'])]
    public function duplicate(
        ...
        return $this->renderForm($request, $project, $category, $values, $catalog, false, null, [], duplicate: true);
    }
    ...
    private function renderForm(
        ...
        $formAction = $edit && null !== $record
            ? $this->generateUrl('backend_emission_edit_material_v1', array_merge(['id' => $record->getId()], $backQuery))
            : $this->generateUrl('backend_emission_new_material_v1', $backQuery);

        return $this->render('backend/emission/material_v1_form.html.twig', [
            'project' => $project,
            'category' => $category,
            'edit' => $edit,
            'duplicate' => $duplicate,
    ...
    {
        return 'endDate cannot be before startDate.' === $exception->getMessage() ? 'invalid_date_range' : 'invalid_input';
    }
    ...
    {
        $category = $repository->findOneBy(['name' => 'Materiales']);
        if (!$category || !$category->isEnabledInEmissionCalculator()) {
            throw $this->createNotFoundException('Material category not found.');
        }
